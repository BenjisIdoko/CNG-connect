...
function stationRow(s: GasStation) {
  return {
    ...
    status: s.status === 'unknown' ? 'full' : s.status, // stations.status has no 'unknown' option (see CHECK)
    status_label: s.statusLabel,
    cng_price: s.cngPrice,
    price_trend: s.priceTrend,
    pump_pressure: s.pumpPressure,
    busy_estimate: s.busyEstimate,
    last_updated: s.lastUpdated,
    verified_by_community: s.verifiedByCommunity,
    is_picng_accredited: s.isPiCngAccredited ?? true,
    operator: s.operator,
    phone: s.phone,
    lat: s.lat,
    lng: s.lng,
  ...
  for (const s of INITIAL_STATIONS) {
    if (s.reports && s.reports.length > 0) {
      const rows = s.reports.map((r) => ({
        ...
        status: r.status === 'unknown' ? 'full' : r.status,
